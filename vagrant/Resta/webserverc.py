    ...
    def do_GET(self):
        try:
            if self.path.endswith("/restaurants"):
                ...
                for rname in result:
#                    print rname[0]
                    theid = session.query(Restaurant.id).filter_by(name = rname[0]).one()
                    output += rname[0] + "<br>"
                    output += "<a href=/restaurants/"+str(theid[0])+"/edit>Edit</a><br>"
                    output += "<a href=/restaurants/"+str(theid[0])+"/delete>Delete</a><br>"                    
                    output += "<br>"
                output += "</body></html>"
...
#            print "self.path ==> ", self.path[13:].find('/')
            ...
            if nextslash != -1 and self.path.endswith('/edit'): 
...
#                if int(string) == True:
            ...
            if nextslash != -1 and self.path.endswith('/delete'):
                string = self.path[13:13+nextslash]
                thename = session.query(Restaurant.name).filter_by(id = int(string)).one()
                print "Integer of this string is : ", int(string)
#                if int(string) == True:
                self.send_response(200)
                self.send_header('Content-type','text/html')
                ...
                output = ""
                output += "<html><body>"
                output += "<h1>Are you sure you want to delete %s ?</h1>" %thename
                output += "<form method = 'POST' enctype='multipart/form-data' action='%s'>" %self.path 
    ...
    def do_POST(self):
        try:
            ...
            nextslash = self.path[13:].find('/')
            if nextslash != -1 and int(self.path[13:13+nextslash]) and self.path.endswith('/edit'):
#                print "Integer following"
            ...
            if nextslash != -1 and int(self.path[13:13+nextslash]) and self.path.endswith('/delete'):
                string = self.path[13:13+nextslash]
                print "deleted string is ", int(string)

                ctype, pdict = cgi.parse_header(
                    self.headers.getheader('content-type'))

                deleted = session.query(Restaurant).filter_by(id = int(string)).one()
